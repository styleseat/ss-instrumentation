...
class MetricAndDims(object):
    ...
    def serialize(self):
        return json.dumps([self.name, sorted([(k, v) for k, v in self.dims.items()])])
...
class SSInstrumentation(object):
    ...
    def put_metrics(self, metrics):
        ...
        May fail in various ways, returns True on success but in most
        contexts you won't want to retry or even bother to check the
        return value.
        """
        metrics_data = []
        for metric in metrics:
            ...
            if "dims" in metric:
                metric_data["Dimensions"] = [{"Name": name, "Value": value} for name, value in metric["dims"].items()]

            metrics_data.append(metric_data)

        try:
            self.client.put_metric_data(Namespace=self.namespace, MetricData=metrics_data)
            return True
        except Exception:
            return False
